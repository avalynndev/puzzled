...
const REVEAL_KEY = "riddle_completed_date";

export function RiddleOfTheDay() {
  ...
  useEffect(() => {
    ...
    const completedDate = localStorage.getItem(REVEAL_KEY);

    if (completedDate === today) {
      setShowAnswer(true);
      setCompleted(true);
      setFeedback("✅ You've already completed today's riddle!");
    }
  }, []);

  const checkAnswer = () => {
    if (!riddle || completed) return;
    ...
    if (
      ...
      const today = new Date().toISOString().split("T")[0];
      localStorage.setItem(REVEAL_KEY, today);
    } else {
